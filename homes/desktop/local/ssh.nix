...
let
  includeCornell =
    if
      lib.hasAttrByPath [ "nixSeal" "secrets" "ianhollow/home/ianmh/cornell-net-id-ssh-config" ] config
    then
      { Include = config.nixSeal.secrets."ianhollow/home/ianmh/cornell-net-id-ssh-config".path; }
    else
      { };
in
{
